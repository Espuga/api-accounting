...
public class VmachinesModel {
    private static List<Map<String, Object>> tableToList(Table data) {
        List<Map<String, Object>> result = new ArrayList<>();

        for(Row row : data) {
            Map<String, Object> aux = new HashMap<>();
            for(String columnName : data.columnNames()) {
                aux.put(columnName, row.getObject(columnName));
            }
            result.add(aux);
        }
    ...
    public static boolean doInvoice(JdbcTemplate jdbcAccounting, InvoiceModel invoiceModel) {
        try {
        	for(Double d : invoiceModel.getPrDisk()) {
            ...
            for(String vlan : invoiceModel.getVlans()) {
                ...
                jdbcAccounting.update(
                    String.format(
                    		"INSERT INTO %s (title, description, amount, data, userId) VALUES (?, ?, ?, ?, (SELECT id FROM users WHERE token = '%s'))", 
